...
from lib.utils import get_vt_censor, get_vt_siblings_url
...
class LoaferEngine:
    ...
    def lf_request(self, url=None, headers=None, path=None, params={}, delay=0, timeout=7):
        try:
            time.sleep(delay)
            if not headers:
                h = self.headers
            ...
            if not url:
                ...
                req = requests.get(url, proxies=None, headers=h, timeout=timeout,
                                   allow_redirects=self.follow_redirect, params=params, verify=False)
